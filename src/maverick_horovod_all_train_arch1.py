...
from horovod.tensorflow.keras.callbacks import BroadcastGlobalVariablesCallback, MetricAverageCallback
...
def configure_gpu():
    print(f"[Rank {hvd.rank()} | Local Rank {hvd.local_rank()}] Starting process...")
    print(f"[Rank {hvd.rank()}] CUDA_VISIBLE_DEVICES before TF import: {os.environ.get('CUDA_VISIBLE_DEVICES')}")

    gpus = tf.config.list_physical_devices('GPU')
    print(f"[Rank {hvd.rank()}] GPUs detected by TF: {gpus}")
    ...
    print(f"[Rank {hvd.rank()} | Local Rank {hvd.local_rank()}] Final GPU assignment complete.")
    print(f"  - CUDA_VISIBLE_DEVICES = {os.environ.get('CUDA_VISIBLE_DEVICES')}")
    print(f"  - TF sees: {tf.config.get_visible_devices('GPU')}")
...
class DataGenerator(keras.utils.Sequence):
    def __init__(self, list_IDs, labels, dataFrameIn, tokenizer, T5Model,
        ...
        # Shard the indices across Horovod workers
        '''
        self.num_samples = len(self.list_IDs)
        self.start_idx = (self.num_samples // hvd.size()) * hvd.rank()
        self.end_idx = self.start_idx + (self.num_samples // hvd.size())
        if hvd.rank() == hvd.size() - 1:
            self.end_idx = self.num_samples
        self.shard_ids = self.list_IDs[self.start_idx:self.end_idx]
        '''
        self.num_samples = len(self.list_IDs)
        self.start_idx = (self.num_samples // hvd.size()) * hvd.rank()
        self.end_idx = self.num_samples if hvd.rank() == hvd.size() - 1 else self.start_idx + (self.num_samples // hvd.size())
        self.shard_ids = self.list_IDs[self.start_idx:self.end_idx]
        # self.labels = self.labels[self.start_idx:self.end_idx]
        self.on_epoch_end()  # Initialize indexes for shard
    ...
    def __getitem__(self, index):
        # Generate one batch of data (Horovod-compatible)
        worker_samples = self.end_idx - self.start_idx
        if (worker_samples % self.batch_size != 0) and ((index + 1) * self.batch_size > worker_samples):
            indexes = self.indexes[index * self.batch_size : worker_samples]
        else:
...
def MaverickArchitecture1(input_shape=201, classes=3, classifier_activation='softmax', **kwargs):
    ...
    optimizer = tf.keras.optimizers.SGD(learning_rate=1e-3 * hvd.size(), momentum=0.85)
...
class OneCycleScheduler(Callback):
    ...
    def on_train_begin(self, logs=None):
        ...
        final_lr = self.lr_max / (self.div_factor * 1e4)
        phase_1_steps = int(self.steps * self.phase_1_pct)
        phase_2_steps = self.steps - phase_1_steps
        self.phases = [
            [CosineAnnealer(lr_min, self.lr_max, phase_1_steps),
             CosineAnnealer(self.mom_max, self.mom_min, phase_1_steps)],
            [CosineAnnealer(self.lr_max, final_lr, phase_2_steps),
             CosineAnnealer(self.mom_min, self.mom_max, phase_2_steps)]
        ]
        self.phase = 0
        self.step_count = 0
        self.set_lr(self.phases[0][0].start)
        self.set_momentum(self.phases[0][1].start)
    ...
    def on_train_batch_end(self, batch, logs=None):
        self.step_count += 1
        if self.phase == 0 and self.step_count >= self.phases[0][0].steps:
            self.phase = 1
    ...
    def get_momentum(self):
        return self.model.optimizer.momentum
...
tf = configure_gpu()
print(f"[Horovod] Rank {hvd.rank()} of {hvd.size()} initialized on local rank {hvd.local_rank()}")
print(f"[Rank {hvd.rank()} | Local Rank {hvd.local_rank()}] GPU setup complete")
print(f"[Rank {hvd.rank()}] Loading tokenizer and T5 model...")
...
lr_schedule = OneCycleScheduler(0.02, steps)
...
y_test_pred_local = predict_set(testData, y_test)

# ============================
# PRINT CLASSIFICATION REPORT
# ============================
y_test_pred_all = hvd.allgather(tf.convert_to_tensor(y_test_pred_local)).numpy()

y_test_true_all = y_test_encoded

if hvd.rank() == 0:
    testData.loc[:,['Maverick_BenignScore','Maverick_DomScore','Maverick_RecScore']]=y_test_pred_all
    testData.loc[:,'classLabelPredicted']=np.argmax(y_test_pred_all,axis=1)
...
print_classification_report(y_test_pred_all, y_test_true_all, "Test")
'''
