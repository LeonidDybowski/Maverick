...
def main ( argv ):
    ...
    resources = 'resources/'
	approvedTranscripts=pd.read_csv(resources + 'gencodeBasicFullLengthTranscriptsConversionTable.txt',sep='\t',low_memory=False)

	canonical=pd.read_csv(resources + 'gnomad211_constraint_canonical_simple.txt',sep='\t',low_memory=False)
	...
	GTEx=pd.read_csv(resources + 'GTEx.V7.tx_medians.021820.tsv',sep='\t',low_memory=False)
	...
	sample=pd.read_csv(base + ".avinput.exonic_variant_function",sep='\t',low_memory=False,header=None,
                       ...
                       dtype={'line':str,'varType':str,'location':str,'hg19_chr':str,'hg19_pos(1-based)':np.int32,'end':np.int32,'ref':str,'alt':str,'genotype':str,'qual':str,'depth':str})
