...
def main ( argv ):
	...
	gnomadAF=pd.read_csv(resources + 'gnomad211_exomes_AFs.txt',sep='\t',low_memory=False,dtype={'hg19_chr':str,'hg19_pos(1-based)':np.int32,'ref':str,'alt':str,'AF':np.float32,'nhomalt':np.int32,'controls_AF':np.float32,'controls_nhomalt':np.int32})
	...
	constraint=pd.read_csv(resources + 'gnomad211_constraint_simple.txt',sep='\t',low_memory=False)
	...
	constraintGeneLevel=pd.read_csv(resources + 'gnomad211_constraint_simple_geneLevel.txt',sep='\t',low_memory=False)
	...
	sample2['CCR']=np.nan
	CCR=pd.read_csv(resources + 'ccrs.enumerated.txt',sep='\t',low_memory=False,dtype={'chrom':str,'pos':np.int32,'ccr_pct':np.float32})
	...
	pext=pd.read_csv(resources + 'gnomAD_pext_values.txt',sep='\t',low_memory=False,dtype={'chr':str,'pos':np.int32,'pext':np.float32})
	...
	gerp=pd.read_csv(resources + 'gerpOnExons.txt',sep='\t',low_memory=False,header=None,names=['chr','pos','gerp'],dtype={'chr':str,'pos':np.int32,'gerp':np.float32})
